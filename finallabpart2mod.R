...
options(scipen=999)
sort(prob1data$gp)
prob1data$gp_1 = runif(dim(prob1data)[1])
#smallset = subset(prob1data, prob1data$gp < 0.1) # gives approx 10% of data
smallset = subset(prob1data, prob1data$gp_1 <= sort(prob1data$gp_1)[round(0.20* dim(prob1data)[1])]) # 20% of data
length(smallset$gp_1)
...
write.csv(fullmodel,"~/FINAL_LAB/fullmodel.csv", sep="\t")
#
...
countiesOfInterest = c("county_nameOldham", "county_nameBoone", "county_nameWoodford", "county_nameFayette", 
                       "county_nameShelby", 
                       "county_nameKenton", "county_nameCampbell", "county_nameFranklin","county_nameScott", "county_nameJefferson")

#Oldham, Boone, Woodford, Fayette, Shelby, Kenton , Campbell, Franklin, Scott & Jefferson.
...
plot(rocFull@x.values[[1]], rocFull@x.values[[1]], type="l", col='gray', xlab="fpr", ylab="tpr", main="ROC Curve" ) 
